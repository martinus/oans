...
- **grouping** (block-level extent search) — `psearch_*`: a lone
  `[####    %      ]` bar on a single line, no worker lines, no ETA.
...
[8] idle
                                              ← one blank line
✔ scanning   ⠹ hashing   · grouping   · dedupe   · done
[███████████████░░░░░░░░░░]  62%  ·  ETA ~1m20s
...
2. **One blank line.**
3. **Stage line** — the five words `scanning hashing grouping dedupe done`, each
   prefixed by a spinner (running), a tick (finished), or a dim dot (not started).
4. **Progress bar + ETA** — for the current stage.
...
- **Label with a 1-based counter, not the pid.** `[1]`…`[N]` = the slot's index
  in `pscan.threads`. The pid (`tid`) stays in the struct for debugging but is
  never shown. *(Today the line prints `[<tid>]`.)*
- Per-status content is unchanged from the current hashing screen:
  - `mapping:` `<path>` `(size: <S>)` — open + fiemap, before the first byte.
  - `hashing:` `<path>` `<done>/<total> (<pct>%)`
  - `wait lock:` / `commit:` `<path>` `(size: <S>)`
  - `deduping:` `<path>` `<done>/<total> (<pct>%)`
  - `idle`
...
Five fixed words in run order. Each gets a **state glyph** in front:
...
Two facts drive this and are worth stating plainly, because they aren't a clean
left-to-right march:
...
2. **grouping is interleaved with dedupe, not before it.** `process_duplicates()`
   loops over generation passes; each pass does: load identical files →
   dedupe whole-file dups → (optionally) load extent hashes + **block-level
   extent search** (`find_additional_dedupe` / `psearch`) → dedupe extents. So
   "grouping" is a sub-activity that recurs inside the dedupe phase, and the
   first real dedupe ioctl can fire *before* the first extent search.

Because of (2), a strict "grouping fully done, then dedupe starts" is not what the
code does. **This is the main design decision in this spec — see §7.**

Recommended rule (**monotonic stages**): a stage's glyph only ever advances
`not-started → running → finished`, never back. Concretely:

- `scanning`: running at launch; ✔ at `pscan_finish_listing`.
- `hashing`: running at launch; ✔ when `scan_files` drains the csum pool.
- `grouping`: running when `process_duplicates` begins its analysis
  (index build / loading identical & extent hashes / extent search); ✔ once the
  first group is actually deduped (`pdedupe.done` goes `0 → 1`, or first reclaimed
  bytes). Later per-pass extent searches keep happening but don't un-tick it —
  they're shown as a dedupe sub-activity on the detail line instead.
- `dedupe`: running from the first deduped group to `pdedupe_end`; ✔ at the end.
- `done`: ✔ once the whole run finishes (final screen, §6).

This keeps the indicator legible (no flicker) at the cost of a small honesty gap:
grouping's tick means "we've started acting on groups", not "no more searching
will occur". §7 lists the alternative if you'd rather the spinner track the true
current activity per pass.
...
the scan∥hash overlap that's `hashing`; during analysis it's `grouping`; then
`dedupe`.
...
- Fixed inner width (proposal: `min(40, cols-someslack)`), `█` filled / `░` empty;
  ASCII fallback `[###------]`. A fractional trailing cell uses the 1/8 block ramp
  `▏▎▍▌▋▊▉` on UTF-8 for smoothness (optional; plain `█`/`░` is fine for v1).
- Percentage always shown. ETA shown only once measurable (`> 1s` elapsed and
  non-zero progress), formatted with the existing `human_duration`.
...
| grouping  | `search_processed / search_total`     | linear extrapolation from elapsed            |
...
- **grouping**: `Grouping: searching extents · 4,200 / 9,800 files`
- **dedupe**: `Deduping 512 / ~1,900 groups · batch 2/5 · reclaimed 3.4 GiB`
  (the per-pass extent search, when it recurs, shows here as
  `· searching extents 4.2k/9.8k` so the recurrence is visible without un-ticking
  the stage).

## 7. The one open design decision — grouping vs dedupe

Because grouping recurs inside the dedupe loop (§4.2), pick how the stage
indicator treats it:

- **Option A — monotonic (recommended, specced above).** `grouping` ticks the
  first time a group is deduped and never un-ticks; later per-pass searches show
  only on the detail line. Legible, no flicker. Downside: the tick slightly
  overstates "grouping is finished".
- **Option B — activity-driven.** Tag every `pdedupe_set_activity()` string as
  either *grouping* or *deduping*; the spinner lights whichever category is
  currently active, so it flips back to `grouping` each pass during the extent
  search. Most truthful moment-to-moment. Downside: `grouping`/`dedupe` glyphs
  flicker between spinner and tick across passes (non-monotonic), which reads as
  "going backwards".
- **Option C — restructure.** Hoist all extent searching ahead of all deduping so
  the stages really are sequential. Cleanest UI, biggest code change, and it
  fights the cross-pass pipelining that `DEDUPE_FILES_PER_PASS` exists to keep
  full — likely a perf regression. Not recommended.

I recommend **A**. Tell me if you'd rather B (I think B's flicker is worse than
A's small imprecision) — or if you want grouping dropped as a top-level word and
folded into the dedupe detail line entirely (a 4-stage line:
...
- Add a single `enum stage { STAGE_SCAN, STAGE_HASH, STAGE_GROUP, STAGE_DEDUPE,
  STAGE_DONE }` with a per-stage state (`pending/running/done`) array, advanced by
  the existing lifecycle hooks (`pscan_finish_listing`, `pscan_join`,
  `pdedupe_begin`, `pdedupe_group_done`, `pdedupe_end`, `psearch_run/join`). No
  new threads — the existing single printer thread renders all of it.
- Fold `print_scan_progress` / `print_dedupe_progress` / the standalone
  ...
  `search_processed/total` into the shared renderer for the grouping stage (it
  already does this when `pdd.phase` is set — extend it to always).
...
- New rendering: the stage line, the block progress bar, and the indeterminate
  (pulsing) bar for the pre-listing-complete window.
